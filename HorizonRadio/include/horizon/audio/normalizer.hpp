...
public:
    ...
    void set_target_rms(float v) noexcept {
        target_rms_ = std::clamp(v, 0.01f, 0.5f);
    }
    void set_max_gain(float v) noexcept {
        max_gain_ = std::clamp(v, 1.0f, 32.0f);
    }
    void set_min_gain(float v) noexcept {
        min_gain_ = std::clamp(v, 0.01f, 1.0f);
    }
    void set_peak_threshold(float v) noexcept {
        peak_threshold_ = std::clamp(v, 0.5f, 1.0f);
    }
    ...
    void process_stereo(float& l, float& r) noexcept {
        ...
        const float     needed_lim    = peak > peak_threshold_ ? peak_threshold_ / peak : 1.0f;
    ...
    float current_limiter_gain() const noexcept {
        return limiter_gain_;
    }
