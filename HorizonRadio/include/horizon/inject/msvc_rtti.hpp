...
public:
    ...
    [[nodiscard]] std::optional<TypeDescriptor> find_type_descriptor(std::string_view mangled_name) const;

    // Find the COL referencing the given TypeDescriptor by RVA.
    [[nodiscard]] std::optional<CompleteObjectLocator> find_complete_object_locator(TypeDescriptor td) const;

    // Find the vtable whose [-1] slot is the absolute address of `col`.
    [[nodiscard]] std::optional<Vtable> find_vtable(CompleteObjectLocator col) const;
