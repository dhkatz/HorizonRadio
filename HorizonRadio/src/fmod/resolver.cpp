...
Fn resolve_one(const horizon::inject::PeImage& image,
               const SignaturePattern& sig,
               bool& found_out) {
    found_out = false;
...
FmodResolver::FmodResolver(const horizon::inject::PeImage& image, SignatureSet sigs)
    : image_(image), sigs_(sigs) {}
