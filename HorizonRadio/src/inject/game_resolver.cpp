...
GameResolver::GameResolver(const horizon::inject::PeImage& image)
    : image_(image) {}

void* GameResolver::resolve_global_via_rip_load(std::string_view pattern,
                                                std::ptrdiff_t mov_offset_in_match) {
    if (pattern.empty()) return nullptr;

    const std::byte* match = horizon::inject::find_pattern(image_.text(), pattern);
    ...
    if (static_cast<std::uint8_t>(mov[0]) != 0x48 ||
        static_cast<std::uint8_t>(mov[1]) != 0x8B ||
    ...
    auto* mov_end = reinterpret_cast<const std::byte*>(mov) + 7;
    auto target_addr = reinterpret_cast<std::uintptr_t>(mov_end) +
                       static_cast<std::intptr_t>(disp32);
...
std::vector<const void*> find_instances_in_heap_arenas(
    ...
    std::size_t min_region_bytes,
    std::size_t max_hits) {
    std::vector<const void*> matches;
    if (expected_vtable == nullptr) return matches;
    ...
    while (addr < max_addr && matches.size() < max_hits) {
        ...
        const bool is_heap_shaped =
            mbi.State == MEM_COMMIT &&
            mbi.Type == MEM_PRIVATE &&
            prot_ok &&
            !overlaps_module &&
            region_size >= min_region_bytes &&
            region_size <= kMaxRegionBytes;

        if (is_heap_shaped) {
            // 16-byte aligned start, 8-byte aligned end (so the final
            // 24-byte read of slot/use/weak/inner-vtable stays in
            // bounds).
            auto* scan = reinterpret_cast<const std::byte*>(
                (region_addr + 15) & ~std::uintptr_t{15});
            const auto* scan_end = reinterpret_cast<const std::byte*>(
                reinterpret_cast<std::uintptr_t>(region_end) & ~std::uintptr_t{7});
            for (; scan + 24 <= scan_end && matches.size() < max_hits; scan += 16) {
                const auto* slot = reinterpret_cast<const std::uintptr_t*>(scan);
                if (safe_read_qword(slot) != vt) continue;
                ...
                if (use_w == 0 || weak_w == 0) continue;
                if (use_w > 0x80 || weak_w > 0x80) continue;
                ...
                const auto inner_vt = safe_read_qword(
                    reinterpret_cast<const void*>(scan + 16));
                if (inner_vt < mod_lo || inner_vt >= mod_hi) continue;
