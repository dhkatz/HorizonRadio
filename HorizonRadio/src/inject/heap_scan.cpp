...
bool region_is_scannable(const MEMORY_BASIC_INFORMATION& mbi) {
    ...
    if (mbi.Type != MEM_PRIVATE)
        return false; // skip MEM_IMAGE/MEM_MAPPED
    if (mbi.Protect & kUnreadableFlags)
        return false;
    if (!(mbi.Protect & kReadableProtectMask))
        return false;
    return true;
}
...
std::vector<const void*> find_heap_instances(const void* target) {
    ...
    find_heap_instances_streaming(target, matches, [] { return true; }, [](std::size_t) {});
    return matches;
}
