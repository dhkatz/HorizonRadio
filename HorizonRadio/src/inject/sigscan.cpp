...
PeImage::PeImage(HMODULE module) {
    ...
    for (WORD i = 0; i < section_count; ++i, ++section) {
        ...
        if (std::strncmp(name, ".text", IMAGE_SIZEOF_SHORT_NAME) == 0) {
            ...
            pdata_ = {
                reinterpret_cast<const RUNTIME_FUNCTION*>(ptr),
...
Pattern compile_pattern(std::string_view s) {
    ...
    auto it = s.begin();
    while (it != s.end()) {
        if (std::isspace(static_cast<unsigned char>(*it))) { ++it; continue; }
...
const std::byte* find_pattern(std::span<const std::byte> haystack, const Pattern& p) {
    if (p.bytes.empty() || haystack.size() < p.bytes.size()) return nullptr;
...
PatternSet compile_pattern_set(std::string_view ida_style) {
    ...
    for (std::size_t i = 0; i <= ida_style.size(); ++i) {
        if (i == ida_style.size() || ida_style[i] == '|') {
            ...
            if (any) set.alternatives.push_back(compile_pattern(chunk));
            start = i + 1;
        }
    }
    return set;
...
bool match_pattern_set_at(std::span<const std::byte> haystack, const PatternSet& set) {
    for (const auto& p : set.alternatives) {
        if (haystack.size() < p.bytes.size()) continue;
        bool ok = true;
        for (std::size_t j = 0; j < p.bytes.size(); ++j) {
            if (p.mask[j] && haystack[j] != p.bytes[j]) { ok = false; break; }
        }
        if (ok) return true;
    }
    return false;
...
std::vector<const std::byte*> find_lea_targeting(
    ...
    std::vector<const std::byte*> out;
    if (targets.empty() || text.size() < 7) return out;

    const auto* p = text.data();
    const auto* limit = p + text.size() - 7;
    for (; p <= limit; ++p) {
...
std::uint32_t resolve_primary_function_rva(const PeImage& image,
    ...
    auto rf_at = [&](std::uint32_t begin_rva) -> const RUNTIME_FUNCTION* {
        ...
        auto it = std::lower_bound(pdata.begin(), pdata.end(), begin_rva,
            [](const RUNTIME_FUNCTION& rf, std::uint32_t rva) {
                return rf.BeginAddress < rva;
            });
        if (it == pdata.end() || it->BeginAddress != begin_rva) return nullptr;
        return &*it;
    };
    ...
    for (int depth = 0; depth < kMaxDepth; ++depth) {
        ...
        if (rf == nullptr) return current;  // not in .pdata at all; best effort

        // Bounds-check the UnwindData pointer.
        if (rf->UnwindData == 0 || rf->UnwindData >= image.image_size()) return current;
        const auto* unwind = reinterpret_cast<const std::uint8_t*>(
            image.base() + rf->UnwindData);

        const auto version_flags = unwind[0];
        const auto flags = static_cast<std::uint8_t>(version_flags >> 3);
        if ((flags & kUnwFlagChainInfo) == 0) return current;  // we're at the primary
        ...
        if (after_codes & 0x2) after_codes += 2;  // pad to DWORD
        ...
        if (chained_rva + sizeof(RUNTIME_FUNCTION) > image.image_size()) return current;

        const auto* chained = reinterpret_cast<const RUNTIME_FUNCTION*>(unwind + after_codes);
        if (chained->BeginAddress == 0 || chained->BeginAddress == current) return current;
...
AnchorResolution diagnose_function_by_anchor(const PeImage& image,
    ...
    for (const auto* lea : leas) {
        const auto rva = static_cast<std::uint32_t>(
            reinterpret_cast<std::uintptr_t>(lea) - base);
        const auto chunk_rva = enclosing_function_rva(image.pdata(), rva);
        if (chunk_rva == 0) continue;
        ...
        if (fn < text_start || fn >= text_end) continue;
        if (std::find(enclosing.begin(), enclosing.end(), fn) == enclosing.end()) {
    ...
    for (const auto* fn : enclosing) {
        ...
        if (match_pattern_set_at(tail, prologue)) matches.push_back(fn);
    }
...
const std::byte* find_function_by_pattern(const PeImage& image,
    ...
    if (!image.valid() || set.alternatives.empty()) return nullptr;
    ...
    for (const auto* p = text_start; p < text_end; ++p) {
        std::span<const std::byte> tail{p, static_cast<std::size_t>(text_end - p)};
        if (match_pattern_set_at(tail, set)) {
            if (++seen > 1) return nullptr;
            match = p;
        }
