...
PcmPipeServer::~PcmPipeServer() { stop(); }

void PcmPipeServer::start(PcmCallback on_pcm) {
    bool expected = false;
...
void PcmPipeServer::run() {
    ...
    while (running_.load(std::memory_order_acquire)) {
        ...
        while (running_.load(std::memory_order_acquire)) {
            ...
            pending += got;
            if (pending < kChunkBytes) continue;  // partial; keep filling
            if (on_pcm_) on_pcm_(buf, kChunkFrames);
            pending = 0;
        }
